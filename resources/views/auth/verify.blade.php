...
<div class="mt-3 text-center">
    <svg version="1.1" xmlns:x="&ns_extend;" xmlns:i="&ns_ai;" xmlns:graph="&ns_graphs;" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 98 98" style="height: 120px;" xml:space="preserve">
        <style type="text/css">
            .st0{fill:#3b434e;}
            .st1{fill:#1abc9c;}
            .st2{fill:#3b434e;stroke:#1abc9c;stroke-width:2;stroke-miterlimit:10;}
            .st3{fill:none;stroke:#3b434e;stroke-width:2;stroke-linecap:round;stroke-miterlimit:10;}
        </style>
        <g i:extraneous="self">
    ...
    @if (session('resent'))
        <div class="alert alert-success" role="alert">
            {{ __('A fresh 6-digits token has been sent to your phone number.') }}
        </div>
    @endif
    <h3>Hurray !</h3>
    <p class="text-muted font-14 mt-2"> 
        {{ __('Before proceeding, please check your phone for a verification token.') }}
        {{ __('If you did not receive the token') }},
        <form class="d-inline" method="POST" action="{{ route('verificationToken.store') }}">
            @csrf
            <button type="submit" class="btn btn-link p-0 m-0 align-baseline">{{ __('click here to request another') }}</button>.
        </form>
    </p>
</div>
<form class="mt-4" action="{{ route('verificationToken.verify') }}" method="post">
    @csrf
    @method('PATCH')
    ...
    <button class="btn btn-block btn-danger waves-effect waves-light" type="submit">Verify</button>
</form>
@endsection
